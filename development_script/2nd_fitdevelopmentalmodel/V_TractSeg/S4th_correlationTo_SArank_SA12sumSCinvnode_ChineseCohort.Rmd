---
title: "S4th_correlationTo_SArank_SA12sumSCinvnode_ChineseCohort"
...
gamresult$sig <- (gamresult$pfdr < 0.05) # CV75 39 sig edges
...
meanderv2_c_control_distance <- residuals(lm(meanderv2~EuricDistance, data=gamresult))
#corr.test(meanderv2_control_distance, gamresult$EuricDistance, method = "pearson") # r=0
gamresult$meanderv2_c_control_distance[!is.na(gamresult$meanderv2)] <- meanderv2_c_control_distance
...
if (CVthr==75){
  lmthr <- max(abs(correlation.df$meanderv2), na.rm=T)
  ggplot(data=correlation.df)+
    ...
    scale_y_continuous(breaks = c(-0.002, 0, 0.002, 0.004))+
    theme_classic()+mytheme
}else{
  lmthr <- max(abs(correlation.df$meanderv2), na.rm=T)
  ggplot(data=correlation.df)+
    geom_point(aes(x=SCrank, y=meanderv2, color=meanderv2), size=5.8)+
    geom_smooth(aes(x=SCrank, y=meanderv2),linewidth=2, method ="lm", color="black")+
    scale_color_distiller(type="seq", palette = "RdBu", direction = -1, limits=c(-lmthr, lmthr))+
    labs(x="S-A connectional axis rank", y="Second derivative")+
    scale_y_continuous(breaks = c(-0.002, 0, 0.002, 0.004), labels = c(-3, 0, 3))+
    theme_classic()+mytheme
}
...
for (i in c(1,5)){
  ...
  }else if (computevar=="meanderv2"){
    lmthr <- max(abs(gamresult$meanderv2), na.rm=T)
    Fig<-ggplot(data =matrixtmp.df.melt)+
      geom_tile(aes(x=variable, y=nodeid, fill = value, color=value))+
