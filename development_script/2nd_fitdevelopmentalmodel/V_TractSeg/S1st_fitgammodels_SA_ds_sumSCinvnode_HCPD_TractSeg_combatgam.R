## HCP-D (ComBat-GAM) | TractSeg (major-bundle) | Fit GAM models per edge (raw + scaled)
##
## Runnable version adapted from:
##   V_TractSeg/S1st_fitgammodels_SA_ds_sumSCinvnode_HCPD.R
##
## Default input (project-relative):
## - outputs/results/combat_gam/hcpd/SCdata_SA12_CV75_sumSCinvnode.TractSeg.combatgam.rds
##
## Outputs (project-relative):
## - outputs/intermediate/2nd_fitdevelopmentalmodel/hcpd/tractseg/combat_gam/CV75/*
...
if (length(missing_required) > 0) stop("Missing required columns in TractSeg SCdata: ", paste(missing_required, collapse = ", "))
...
if (length(sc_cols) != elementnum) {
  stop("Expected ", elementnum, " SC edge columns, got ", length(sc_cols), ".")
}
...
out_gamresults_raw <- file.path(interfileFolder, paste0("gamresults", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_TractSeg.rds"))
out_gammodel_raw <- file.path(interfileFolder, paste0("gammodel", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_TractSeg.rds"))
out_plotdatasum_df <- file.path(interfileFolder, paste0("plotdatasum.df_SA", ds.resolution, "_sumSCinvnode_CV", CVthr, "_TractSeg.rds"))
out_scdata_diw <- file.path(interfileFolder, paste0("SCdata.diw_SA", ds.resolution, "CV", CVthr, "_TractSeg.rds"))
out_gammodel_scaled <- file.path(interfileFolder, paste0("gammodel", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_scale_TRUE_TractSeg.rds"))
out_gamresults_scaled <- file.path(interfileFolder, paste0("gamresults", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_scale_TRUE_TractSeg.rds"))
...
if (need_raw) {
  ...
  if (length(ok_idx) < n_edges) {
    ...
    writeLines(failed, file.path(interfileFolder, paste0("failed_edges_TractSeg_sumSCinvnode_CV", CVthr, ".txt")))
    saveRDS(ok_idx, file.path(interfileFolder, paste0("ok_edge_index_TractSeg_sumSCinvnode_CV", CVthr, ".rds")))
    message("[INFO] Successful edges: ", length(ok_idx), " / ", n_edges)
    stop("GAM fits failed for ", length(failed), " edges; see failed_edges_TractSeg_sumSCinvnode_CV", CVthr, ".txt")
  }
...
if (should_run(out_gammodel_scaled) || should_run(out_gamresults_scaled)) {
  ...
  if (length(scaled_ok2) < length(sc_cols_scaled_ok)) {
    ...
    writeLines(failed_scaled, file.path(interfileFolder, paste0("failed_edges_TractSeg_sumSCinvnode_CV", CVthr, "_scale_TRUE.txt")))
    message("[INFO] Successful scaled edges: ", length(scaled_ok2), " / ", length(sc_cols_scaled_ok))
    stop("Scaled GAM fits failed for ", length(failed_scaled), " edges; see failed_edges_TractSeg_sumSCinvnode_CV", CVthr, "_scale_TRUE.txt")
  }
