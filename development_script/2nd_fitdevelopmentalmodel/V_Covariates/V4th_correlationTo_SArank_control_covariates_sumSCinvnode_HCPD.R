...
##
## Spearman correlations between GAM-derived statistics and S-A connectional axis
## rank, plus scatter plots and matrix graphs (tiff only for matrices).
##
## Inputs:
##   outputs/intermediate/2nd_fitdevelopmentalmodel/hcpd/covariates/<cov_tag>/combat_gam/CV75/gamresults*_scale_TRUE.rds
## Outputs:
##   outputs/results/2nd_fitdevelopmentalmodel/hcpd/covariates/<cov_tag>/combat_gam/CV75/SCrank_correlation_summary.csv
##   outputs/figures/2nd_fitdevelopmentalmodel/hcpd/covariates/<cov_tag>/combat_gam/CV75/correlation_sumSCinvnode_SCrank/*.(tiff,pdf)
##   outputs/figures/.../Matrix12_sumSCinvnode_gamstats_Age8_22/*.tiff
##
## Logs always print r and p values.
...
p_md <- ggplot(data = cor_md) +
  geom_point(aes(x = SCrank, y = meanderv2, color = SCrank), size = 5.5, alpha = 0.9) +
...
if (make_matrix_graphs) {
  ...
  computevar.list2 <- c("partialRsq", "increase.onset", "increase.offset", "peak.increase.change", "meanderv2")
