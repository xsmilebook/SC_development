...
Cogvar_base <- "nihtbx_fluidcomp_fc_base"
...
intermediateFolder <- file.path(project_root, "outputs", "intermediate", "5th_cognition", "abcd", "fluid_fc")
...
SCdata[, sc_cols] <- lapply(SCdata[, sc_cols, drop = FALSE], as.numeric)

dataname <- "SCdata"
smooth_var <- "age"
int_var <- Cogvar_base
covariates <- if (mode == "meanfd_only") "mean_fd" else "sex+mean_fd"
knots <- 3
set_fx <- TRUE
increments <- 100
stats_only <- FALSE
