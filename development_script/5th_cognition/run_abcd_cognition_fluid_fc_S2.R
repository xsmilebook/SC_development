...
resultFolder <- file.path(project_root, "outputs", "results", "5th_cognition", "abcd", "fluid_fc")
FigureFolder <- file.path(project_root, "outputs", "figures", "5th_cognition", "abcd", "fluid_fc")
...
in_rds <- file.path(
  resultFolder,
  paste0("SC_Cog_results_", Cogvar, "_CV", CVthr, "_fluid_fc", variant_suffix, ".rds")
)
if (!file.exists(in_rds)) {
  stop("Missing S1 output: ", in_rds, "\nRun first: Rscript development_script/5th_cognition/run_abcd_cognition_fluid_fc_S1.R")
}
...
## 1) "component" plot
comp_plot <- data.frame(category = "NIH Toolbox fluid cognition (fully-corrected)", value = 1)
...
ggsave(
  file.path(FigureFolder, paste0("NTB_fluidcognition_fc_component", variant_suffix, ".tiff")),
  p_comp,
  width = 16,
  height = 6,
  units = "cm",
...
ggsave(
  file.path(FigureFolder, paste0("NTB_fluidcognition_fc_component", variant_suffix, ".pdf")),
  p_comp,
  dpi = 600,
  width = 16,
  height = 6,
