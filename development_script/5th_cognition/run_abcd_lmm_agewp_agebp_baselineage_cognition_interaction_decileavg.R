...
resultFolder <- file.path(
  project_root, "outputs", "results", "5th_cognition", "abcd",
  "age_wp_bp_lmm_baselineage_interaction_decileavg"
)
FigureFolder <- file.path(
  project_root, "outputs", "figures", "5th_cognition", "abcd",
  "age_wp_bp_lmm_baselineage_interaction_decileavg"
)
...
if (!file.exists(cog_rds)) {
  stop(
    "Missing cognition baseline input: ", cog_rds,
...
SCcog <- ensure_eventname(SCcog, "SC cognition data")
...
Cogdf <- extract_baseline_cov(SCcog, value_col = Cogvar, out_col = Cogvar_base)
...
base_by_sub <- tapply(SCdata[[Cogvar_base]], SCdata$subID, function(x) x[which(!is.na(x))[1]])
...
if (need_refit || !file.exists(out_rds)) {
  ...
  edge_fit_list <- lapply(
    ...
    function(edge) {
      fit_outcome_models(
        data_all = SCdata, y_col = edge, cov_col = Cogvar_base,
  ...
  decile_fit_list <- lapply(
    ...
    function(dec_col) {
      fit_outcome_models(
        data_all = SCdata_dec, y_col = dec_col, cov_col = Cogvar_base,
        q_low = q10, q_high = q90,
        age_wp_seq = age_wp_seq, age_bp_seq = age_bp_seq,
        age_wp_ref = age_wp_ref, age_bp_ref = age_bp_ref,
        sex_ref = sex_ref, mean_fd_ref = mean_fd_ref, subid_ref = subid_ref
  ...
  write.csv(
    edge_summary,
    file.path(resultFolder, paste0("edge_summary_baselineage_interaction_", Cogvar_base, "_CV", CVthr, ".csv")),
    row.names = FALSE
  )
  write.csv(
    decile_summary,
...
line_types <- c(low = "dashed", high = "solid")
