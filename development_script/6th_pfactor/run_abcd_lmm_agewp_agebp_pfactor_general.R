...
resultFolder <- file.path(project_root, "outputs", "results", "6th_pfactor", "abcd", "age_wp_bp_lmm")
FigureFolder <- file.path(project_root, "outputs", "figures", "6th_pfactor", "abcd", "age_wp_bp_lmm")
...
message("[INFO] Fitting LMM (pfactor) with age_wp + age_bp")
...
if (length(zero_edges) > 0) {
  writeLines(
    zero_edges,
    con = file.path(resultFolder, paste0("edges_rand_agewp_all_zero_", Pvar, "_CV", CVthr, ".txt"))
  )
}
...
ggsave(file.path(FigureFolder, paste0("scatter_age_bp_partialR2_vs_SCrank_", Pvar, "_CV", CVthr, ".tiff")),
       scatterFig.bp, width = 15, height = 15, units = "cm", bg = "transparent")
ggsave(file.path(FigureFolder, paste0("scatter_age_bp_partialR2_vs_SCrank_", Pvar, "_CV", CVthr, ".pdf")),
       scatterFig.bp, width = 15, height = 15, units = "cm", bg = "transparent")
...
plot_matrix_sig(
  ...
  file.path(FigureFolder, paste0("matrix_personal_agewp_tvalue_low10_high10_", Pvar, "_CV", CVthr))
)

message("[INFO] age_wp personal slope vs pfactor correlation matrix + S-A axis")
mat_cor <- vec_to_mat(res_df$personal_cor, ds = 12)
sig_cor <- vec_to_mat(res_df$personal_cor_p_fdr < 0.05, ds = 12)
plot_matrix_sig(
  mat_cor,
  sig_cor,
  "Personal slope vs pfactor (r)",
  file.path(FigureFolder, paste0("matrix_personal_agewp_corr_", Pvar, "_CV", CVthr))
)

SCrank.df.cor <- SCrankcorr(res_df, "personal_cor", 12, dsdata = FALSE)
saveRDS(SCrank.df.cor, file.path(resultFolder, paste0("SCrankcorr_personal_agewp_corr_", Pvar, "_CV", CVthr, ".rds")))
...
scatterFig.cor <- ggplot(SCrank.data.cor) +
  ...
  labs(x = "S-A connectional axis rank", y = "Personal slope vs pfactor (r)")
ggsave(file.path(FigureFolder, paste0("scatter_personal_agewp_corr_vs_SCrank_", Pvar, "_CV", CVthr, ".tiff")),
       scatterFig.cor, width = 15, height = 15, units = "cm", bg = "transparent")
ggsave(file.path(FigureFolder, paste0("scatter_personal_agewp_corr_vs_SCrank_", Pvar, "_CV", CVthr, ".pdf")),
       scatterFig.cor, width = 15, height = 15, units = "cm", bg = "transparent")
...
ggsave(
  file.path(FigureFolder, paste0("bar_personal_agewp_deciles_low_high_", Pvar, "_CV", CVthr, ".tiff")),
  bar_fig, width = 18, height = 12, units = "cm", bg = "transparent"
)
ggsave(
  file.path(FigureFolder, paste0("bar_personal_agewp_deciles_low_high_", Pvar, "_CV", CVthr, ".pdf")),
  bar_fig, width = 18, height = 12, units = "cm", bg = "transparent"
)
