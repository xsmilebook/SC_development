...
run_all <- function() {
  results <- lapply(sc_cols, function(edge) {
    ...
    if (is.finite(y_mean) && is.finite(y_sd) && y_sd > 0) {
      out_sub <- unique(df_edge$subID[which(y < y_mean - 3 * y_sd | y > y_mean + 3 * y_sd)])
...
write.csv(
  decile_summary,
  file.path(resultFolder, paste0("personal_slope_decile_low_high_", Pvar_base, "_CV", CVthr, out_suffix, ".csv")),
  row.names = FALSE
)
