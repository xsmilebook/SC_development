## HCP-D (ComBat-GAM) | Yeo7/Yeo17 | Age-separated GAM analysis at flip age
##
## Purpose:
...
## - outputs/intermediate/2nd_fitdevelopmentalmodel/chinese/yeo/Yeo{7,17}/combat_gam/CV75/SCdata.diw_Yeo{7,17}CV75.rds
## - outputs/results/4th_changerate_SAcorr/chinese/yeo/Yeo{7,17}/combat_gam/CV75/alignment_summary_flip_age.csv
##
## Outputs:
## - outputs/intermediate/4th_changerate_SAcorr/chinese/yeo/Yeo{7,17}/combat_gam/CV75/gamresults*_Yeo*_*.rds
## - outputs/figures/4th_changerate_SAcorr/chinese/yeo/Yeo{7,17}/combat_gam/CV75/correlation_sumSCinvnode_SCrank_{younger,older}/*.tiff + *.pdf
##
## Usage:
##   Rscript --vanilla development_script/4th_changerate_SAcorr/V_Yeo_network/V2nd_fitgammodels_Yeo_ageseperate_HCPD_combatgam.R --yeo=7
##   Rscript --vanilla .../V2nd_fitgammodels_Yeo_ageseperate_HCPD_combatgam.R --yeo=17 --n_edges=10 --force=1
...
inter2_dir <- file.path(project_root, "outputs", "intermediate", "2nd_fitdevelopmentalmodel", dataset, "yeo", paste0("Yeo", yeo), "combat_gam", paste0("CV", CVthr))
scdata_diw_rds <- if (!is.null(args$scdata_diw_rds)) {
  args$scdata_diw_rds
} else {
  file.path(inter2_dir, paste0("SCdata.diw_Yeo", yeo, "CV", CVthr, ".rds"))
}
...
inter4_dir <- file.path(project_root, "outputs", "intermediate", "4th_changerate_SAcorr", dataset, "yeo", paste0("Yeo", yeo), "combat_gam", paste0("CV", CVthr))
figure_root <- file.path(project_root, "outputs", "figures", "4th_changerate_SAcorr", dataset, "yeo", paste0("Yeo", yeo), "combat_gam", paste0("CV", CVthr))
...
message("[INFO] dataset=", dataset, " yeo=", yeo, " matsize=", matsize, " elementnum=", elementnum, " n_edges=", n_edges)
...
out_young_results <- file.path(inter4_dir, paste0("gamresults", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_younger_Yeo", yeo, ".rds"))
out_old_results <- file.path(inter4_dir, paste0("gamresults", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_older_Yeo", yeo, ".rds"))
out_young_models <- file.path(inter4_dir, paste0("gammodel", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_younger_Yeo", yeo, ".rds"))
out_old_models <- file.path(inter4_dir, paste0("gammodel", elementnum, "_sumSCinvnode_over8_CV", CVthr, "_older_Yeo", yeo, ".rds"))
...
# Younger: partialRsq with outlier filter (match HCPD logic)
...
message(sprintf("[RESULT] dataset=%s yeo=%d younger %s: r=%.5f p=%.3g (n=%d)", dataset, yeo, computevar_y, ct_y$r.spearman[[1]], ct_y$p.spearman[[1]], nrow(gamresult_y)))
...
message(sprintf("[RESULT] dataset=%s yeo=%d older %s: r=%.5f p=%.3g (n=%d)", dataset, yeo, computevar_o, ct_o$r.spearman[[1]], ct_o$p.spearman[[1]], nrow(gamresult_o)))
