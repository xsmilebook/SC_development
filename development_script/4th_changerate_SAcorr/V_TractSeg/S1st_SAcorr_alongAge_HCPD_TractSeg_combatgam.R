## HCP-D (ComBat-GAM) | TractSeg (major-bundle) | Age-resolved alignment with S-A axis
##
## Purpose:
...
## - outputs/results/2nd_fitdevelopmentalmodel/hcpd/tractseg/combat_gam/CV75/derivative.posterior.df.SA12_CV75_TractSeg.rds
##
## Outputs:
## - outputs/results/4th_changerate_SAcorr/hcpd/tractseg/combat_gam/CV75/alignment_*.csv*
## - outputs/figures/4th_changerate_SAcorr/hcpd/tractseg/combat_gam/CV75/Alignment_development/*.tiff + *.pdf
##
## Usage:
##   Rscript --vanilla development_script/4th_changerate_SAcorr/V_TractSeg/S1st_SAcorr_alongAge_HCPD_TractSeg_combatgam.R
##   Rscript --vanilla .../S1st_SAcorr_alongAge_HCPD_TractSeg_combatgam.R --n_draws=200 --force=1
...
figure_dir <- file.path(project_root, "outputs", "figures", "4th_changerate_SAcorr", dataset, "tractseg", "combat_gam", paste0("CV", CVthr), "Alignment_development")
...
in_derivative_posterior <- if (!is.null(args$derivative_posterior_rds)) {
  ...
  file.path(project_root, "outputs", "results", "2nd_fitdevelopmentalmodel", dataset, "tractseg", "combat_gam", paste0("CV", CVthr),
            paste0("derivative.posterior.df.SA12_CV", CVthr, "_TractSeg.rds"))
}
...
message("[INFO] dataset=", dataset, " tractseg matsize=", matsize, " edges=", n_edges)
...
message(sprintf("[RESULT] dataset=%s tractseg n_edges=%d n_draws=%d n_age_points=%d", dataset, n_edges, nrow(corr_mat), length(age_values)))
...
ggsave(file.path(figure_dir, "TractSeg_posDeriv_divweight_corr.tiff"), p_align, dpi = 600, width = 13.5, height = 13.5, units = "cm", bg = "transparent")
ggsave(file.path(figure_dir, "TractSeg_posDeriv_divweight_corr.pdf"), p_align, dpi = 600, width = 13.5, height = 13.5, units = "cm", bg = "transparent")
...
ggsave(file.path(figure_dir, "Agedistribution_0corr_TractSeg.tiff"), p_hist, dpi = 600, width = 6, height = 6, units = "cm", bg = "transparent")
ggsave(file.path(figure_dir, "Agedistribution_0corr_TractSeg.pdf"), p_hist, dpi = 600, width = 6, height = 6, units = "cm", bg = "transparent")
...
if (is_windows) {
  save_svg_if_available(file.path(figure_dir, "TractSeg_posDeriv_divweight_corr.svg"), p_align, width_cm = 13.5, height_cm = 13.5)
  save_svg_if_available(file.path(figure_dir, "Agedistribution_0corr_TractSeg.svg"), p_hist, width_cm = 6, height_cm = 6)
}
