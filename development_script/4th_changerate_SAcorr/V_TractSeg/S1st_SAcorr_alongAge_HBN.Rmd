---
title: "S1st_SAcorr_alongage_HBN"
author: "Xiaoyu Xu"
date: "2024-11-07"
...
age.0.corr.diw.median # 20.38 (CV75)
age.0.corr.diw.CI # 15.82835 21.61241 (CV75)
...
ggplot(data=df.poscorr.diw)+
  ...
  #geom_line(aes(x=zero.corr.window, y=median.loess), color="#B2182B", alpha=1, linewidth=1.6)+
...
df.poscorr.diw$median[df.poscorr.diw$age==min(agerange)] #-0.822
# age = 5
min(agerange)
df.age5 <-as.data.frame(matrix(NA, nrow=1, ncol=ncol(derivative.df)))
names(df.age5)<-names(derivative.df)
for (i in 1:78){
  df.tmp <- derivative.df[derivative.df$label_ID==paste0("SC.", i, "_h"),]
  df.tmp <- df.tmp[df.tmp$age==min(agerange), ]
  df.age5 <- rbind(df.age5, df.tmp)
}
df.age5 <- df.age5[-1,]

SCrankcorr(df.age5, "derivative", 12)
...
df.agemerge <- rbind(df.age5, df.age15, df.age22)
...
minderiv <- min(c(df.age5$derivative, df.age15$derivative, df.age22$derivative))
maxderiv <- max(c(df.age5$derivative, df.age15$derivative, df.age22$derivative))
...
# age 5
Matsize <- 12
matrix_Deriv.age5 <- Matrix12
matrix_Deriv.age5[indexsave12] <- df.age5$derivative
matrix_Deriv.age5[indexup12] <- t(matrix_Deriv.age5)[indexup12]
colnames(matrix_Deriv.age5) <-seq(1, Matsize)
rownames(matrix_Deriv.age5) <-seq(1, Matsize)
matrix_Deriv.age5.df <- as.data.frame(matrix_Deriv.age5)
matrix_Deriv.age5.df$nodeid <- seq(1, Matsize)
matrix_Deriv.age5.df.melt <- melt(matrix_Deriv.age5.df,id.vars=c("nodeid"))
matrix_Deriv.age5.df.melt$variable<-as.numeric(matrix_Deriv.age5.df.melt$variable)
matrix_Deriv.age5.df.melt$nodeid<-0-matrix_Deriv.age5.df.melt$nodeid
ggplot(data =matrix_Deriv.age5.df.melt)+
...
ggsave(paste0(FigureFolder,'/CV', CVthr,   '/Alignment_development/Deri_SA12_diw_Age5.tiff'),  height = 13, width = 15, units = "cm")
...
derivative.df.merge2 <- derivative.df.merge %>% filter(!label_ID==derivative.df.merge$label_ID[which.max(derivative.df.merge$derivative)])

## line plot to present change rate
ggplot(data=derivative.df.merge2)+
