...
- 2026-01-20: 修复 HCP-D 发育模型 S1 GAM 拟合失败：`gamfunction/gamsmooth.R`/`gamfunction/gamderivatives.R` 兼容 `gratia` 不同版本的导数输出列名；移除 `ecostats::anovaPB()` 并改用 `mgcv::anova()` 近似检验以避免并行序列化报错（如 `nbinom2 not found`）。
