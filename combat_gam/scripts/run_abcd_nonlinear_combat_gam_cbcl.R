...
output_rds <- if (length(args) >= 2) args[[2]] else "outputs/results/combat_gam/abcd/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.combatgam_cbcl.rds"
...
colnames(X_nlin) <- "age"
X_lin <- df[, c("sex", "mean_fd", "cbcl_scr_syn_totprob_r"), drop = FALSE]
X_lin <- as.data.frame(lapply(X_lin, normalize_numeric))
colnames(X_lin) <- c("sex", "mean_fd", "cbcl_scr_syn_totprob_r")
batch <- df$siteID
...
out$cbcl_scr_syn_totprob_r <- df$cbcl_scr_syn_totprob_r
