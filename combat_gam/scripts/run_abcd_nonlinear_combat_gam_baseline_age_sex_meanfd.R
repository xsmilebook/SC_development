...
output_rds <- if (length(args) >= 2) args[[2]] else "outputs/results/combat_gam/abcd/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.combatgam_age_sex_meanfd_baseline.rds"
...
needed <- c(sc_cols, "subID", "scanID", "siteID", "age", "sex", "mean_fd", "eventname")
...
colnames(X_nlin) <- "age"
X_lin <- df[, c("sex", "mean_fd"), drop = FALSE]
X_lin <- as.data.frame(lapply(X_lin, normalize_numeric))
colnames(X_lin) <- c("sex", "mean_fd")
batch <- df$siteID
