...
nlongcombat=function(features,knot,ranef,X_nlin,X_lin,batch,use_random=TRUE){
stand=c()
subid=ranef
...
if (use_random) {
  ...
  maxi[,i]=as.matrix(yur$fitted.values) 
  Fitted_site[,i]=model.matrix(yur)[,2]%*%as.matrix(yur$coefficients[2])
  stand[i]=sd(residuals(yur))
}
if (is.na(stand[i]) || stand[i] == 0) {
  stand[i] <- 1
}
Residual[,i]=(y-(maxi[,i]-Fitted_site[,i]))/stand[i]
Residual[is.na(Residual[,i]), i] <- 0
