...
- 生成 ABCD 三套协变量方案的 R2 方差分解图（base/cognition/pfactor）。
- 生成 HCPD 与 Chinese Cohort 的 Raw vs ComBat R2 方差分解图。
...
## 小结
- 已完成 HCPD/Chinese/ABCD 三套 Raw vs ComBat 的 R2 方差分解图，排序与配色保持一致，便于对比站点效应削弱与生物/行为信号保留。
- ABCD 并行与 ComBat 结果稳定性依赖两点：限制 BLAS 线程数、批次效应回加时使用完整 batch dummy 列。
