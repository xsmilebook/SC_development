...
## 追加：修复容器内 `knitr` 安装失败（xfun API 不匹配）
- 现象：构建时报 `Error: object ‘attr’ is not exported by 'namespace:xfun'`，导致 `knitr` lazy-load 失败。
- 修复：在容器中固定 `xfun==0.52`（与现有 R 4.1.3 工作流一致），再安装 `knitr==1.43`，避免 xfun API 漂移导致的构建失败；同时在 `install_version()` 中显式限制 `dependencies` 为 `Depends/Imports/LinkingTo` 并将 xfun/knitr 提前安装，避免因自动拉取 `Suggests` 依赖导致的版本覆盖。
