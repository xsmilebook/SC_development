...
- 给出基于 within-person change 与认知关系的分析方案与 figure（以 `totalstrength` 为例）。
 - 新增 change score（latent change）分析：对两时间点的被试计算 `delta_SC/year`，用线性模型检验 cognition/pfactor 的调制效应，并计算 `beta` 与 S-A rank 的相关。
...
- `gamfunction/lmfunction.R`：
  - 提供 `lm.fit.change()`，在 subject-level change score 上拟合 `change ~ X + age_baseline + sex + mean_fd` 并输出 `beta_int` 等统计量。
  - change score 基于“归一化连接强度比率”（每条边除以 `plotdatasum` 的初始 `fit`）。
- 新增 change score 入口脚本（ABCD）：
  - cognition：`development_script/5th_cognition/run_abcd_change_score_lm_cognition_uncorrected.R`
  - pfactor：`development_script/6th_pfactor/run_abcd_change_score_lm_pfactor_general.R`
- 新增容器 sbatch（由用户提交）：
...
- LMM t value 散点：
  - cognition：`scatter_tvalue_vs_SCrank_lmm_nihtbx_fluidcomp_uncorrected_base_CV75.*`
  - pfactor：`scatter_tvalue_vs_SCrank_lmm_pfactor_GENERAL_CV75.*`
 - change score LM 输出：
   - cognition：`outputs/results/5th_cognition/abcd/change_score_lm/lmresult_change_score_nihtbx_fluidcomp_uncorrected_base_CV75.{rds,csv}`
   - pfactor：`outputs/results/6th_pfactor/abcd/change_score_lm/lmresult_change_score_pfactor_GENERAL_CV75.{rds,csv}`
   - S-A rank 相关：`SCrankcorr_change_score_*`（RDS；Spearman r/p）
   - beta vs S-A rank 散点：`outputs/figures/*/change_score_lm/scatter_beta_vs_SCrank_change_score_*`
   - t value vs S-A rank 散点：`outputs/figures/*/change_score_lm/scatter_tvalue_vs_SCrank_change_score_*`
   - decile change-score 图：`outputs/figures/*/change_score_lm/delta_SC_deciles_vs_*`（含 r/p CSV）
...
- pfactor LMM 默认使用 Nonlinear-ComBat-GAM 的 pfactor 变体输出：
  ...
  - pfactor decile change-score 图：按 `SA12_10.csv` 的 decile 对连接分组；先按 `plotdatasum` 的初始 `fit` 归一化为 ratio，再计算 decile 平均 ratio 的变化率；log 输出每个 decile 的 r/p，并写入 `outputs/results/6th_pfactor/abcd/withinperson_lmm/delta_SC_deciles_vs_pfactor_GENERAL_residualized_rp.csv`。
- change score LM 输入：
  - cognition：纵向 SC `*combatgam_age_sex_meanfd.rds` + baseline cognition `*combatgam_cognition.rds`
  - pfactor：纵向 SC `*combatgam_pfactor.rds`
 - age 线性混合模型（固定效应 + 随机斜率）：
   ...
   - 低/高 10% 组：基于 baseline cognition 划分，分别计算固定效应矩阵与随机效应矩阵并绘制。
