...
- 提供容器版一键提交脚本（默认 72 核，多分区）：`sbatch/run_chinese_devmodel_combatgam_CV75_container.sbatch`
...
- 调整 `S3rd_visualizationfitSCcurves_SA12sumSCinvnode_ChineseCohort_combatgam.R` 与 `S4th_correlationTo_SArank_SA12sumSCinvnode_ChineseCohort_combatgam.R`：
  - S3：默认输出与原始 Rmd 一致（3 张图，各自 tiff+pdf）；示例边图改为可选（`--make_examples=1`）。
  - S4：默认输出与原始 Rmd 一致（`meanderv2_c` tiff；`meanderv2_c_control_distance` tiff+pdf）；matrix graph 仍为可选（`MAKE_MATRIX_GRAPHS=1`）。

### 修复：Chinese devmodel 容器缺少 svglite
- 报错：`there is no package called 'svglite'`（保存 `.svg` 时触发）。
- 处理：Chinese 的 S3/S4 默认不再输出 `.svg`，改为输出 `.pdf`（并保留 `.tiff`），避免额外依赖。
- 同步：`sbatch/run_chinese_devmodel_combatgam_CV75_container.sbatch` 默认改为 72 核，并通过 `SINGULARITYENV_*` 限制 BLAS/OMP 线程数，避免高并行下的 OpenBLAS 线程创建失败。
...
### 修复：容器缺少 svglite 导致 ggsave(.svg) 失败
- 报错：`there is no package called 'svglite'`（发生在 Yeo/TractSeg 的 S3/S4 绘图步骤）
- 处理：将 Yeo/TractSeg 的可复现脚本默认图像输出从 `.svg` 改为 `.pdf`（同时保留 `.tiff`），避免依赖 `svglite`。

### 修复：OpenBLAS 线程创建失败（pthread_create failed）
- 现象：容器作业内出现 `OpenBLAS blas_thread_init: pthread_create failed ... Resource temporarily unavailable`。
- 原因：在 sbatch 分配多核时，OpenBLAS 默认尝试启动大量 BLAS 线程，与脚本本身的并行（`mclapply` 等）叠加导致线程/进程资源耗尽。
- 处理：在 `sbatch/run_hcpd_devmodel_combatgam_CV75_yeo_tractseg_container.sbatch` 中强制设置
  `OPENBLAS_NUM_THREADS=1`、`OMP_NUM_THREADS=1`、`MKL_NUM_THREADS=1`，并通过 `SINGULARITYENV_*` 传入容器。
...
## 补充：HCP-D 派生表缺失 `site` 列的处理

### 背景
- HCP-D 的 Yeo7/Yeo17/TractSeg 等派生 SC 表可能不包含 `site`（批次）列，导致 ComBat-GAM 无法运行。

### 处理策略
- 若输入表缺失 `--batch-col`（默认 `site`），则从 SA12 merge 表按 `subID` 回填：
  `/ibmgpfs/cuizaixu_lab/xuxiaoyu/SC_development/interdataFolder_HCPD/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.merge.rds`
- 已在 `combat_gam/sbatch/hcpd_combat_gam_yeo_tractseg_CV75.sbatch` 中默认传入 `--batch-source-rds`，无需额外操作；如需覆盖可设置 `BATCH_SOURCE_RDS=...`。

## 新增：HCP-D（Yeo7/Yeo17/TractSeg）4th_changerate_SAcorr（对齐曲线 + flip-age 分组 GAM）

### 目标
- 复用 2nd 输出的 posterior derivative，在 1,000 个年龄点上计算 “SC change rate vs S-A axis rank” 的 Spearman rho（posterior 中位数 + 95% CI），并在日志中输出 flip-age（rho≈0）的中位数与 95% CI 等具体数值。
- 对每个版本（Yeo7/Yeo17/TractSeg）进一步按 flip-age 分段拟合 GAM，并输出 partial-Rsq vs S-A rank 的散点与矩阵图，同时在日志中输出相关系数与 p 值。

### 实现
- 新增可复现脚本（project-root 相对路径；图像默认输出 `tiff + pdf`）：
  - Yeo：`development_script/4th_changerate_SAcorr/V_Yeo_network/V1st_SAcorr_alongAge_Yeo_HCPD_combatgam.R`、`development_script/4th_changerate_SAcorr/V_Yeo_network/V2nd_fitgammodels_Yeo_ageseperate_HCPD_combatgam.R`
  - TractSeg：`development_script/4th_changerate_SAcorr/V_TractSeg/S1st_SAcorr_alongAge_HCPD_TractSeg_combatgam.R`、`development_script/4th_changerate_SAcorr/V_TractSeg/S2nd_fitgammodels_TractSeg_ageseperate_HCPD_combatgam.R`
- 新增容器一键提交脚本：`sbatch/run_hcpd_changerate_sacorr_combatgam_CV75_yeo_tractseg_container.sbatch`
  - 依次跑：Yeo7 → Yeo17 → TractSeg（缺失 derivative 时自动补跑 2nd 上游步骤）
  - 日志中包含 `[RESULT]` 行（flip-age 中位数/CI、rho 极值、分组 GAM 的相关系数与 p 值）
