...
- 另外：S3 的交互模型使用 `gamm4(..., random=~(1|subID))`，需要纵向观测（每个 `subID` 至少两条记录）。因此 S3 使用 `*combatgam_age_sex_meanfd.rds`（纵向 SC 数据）作为输入，并从 `demopath/DemodfScreenFinal.csv` 回填 baseline cognition 值；baseline-only 的 `*baseline.rds` 会导致 `lme4` 报错并使所有边拟合失败。
