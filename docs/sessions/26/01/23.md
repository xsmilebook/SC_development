...
- `ABCD_PLOTDATASUM_RDS`（默认指向 `outputs/intermediate/2nd_fitdevelopmentalmodel/abcd/combat_gam/CV75/plotdatasum.df_SA12_sumSCinvnode_siteall_CV75.rds`；可在提交时覆盖）
...
### 追加：variance decomposition 绘图（fluidcomp_fc）
- `combat_gam/scripts/plot_abcd_variance_decomposition.R` 新增 `nihtbx_fluidcomp_fc`（baseline-only）的 Raw vs ComBat 方差分解绘图：
  - 输出前缀：`abcd_variance_decomp_cognition_fluidcomp_fc`
  - Raw 侧：如输入 rds 缺少 `nihtbx_fluidcomp_fc`，会从 `demopath/DemodfScreenFinal.csv` 按 `scanID` 回填（回填失败会报缺列）。
- 修复：为避免图中 `fluidcomp_fc` 图例缺失，脚本对 `fluidcomp_fc` 补齐 palette 映射并设置 `drop=FALSE`，同时将 `nihtbx_fluidcomp_fc` 强制转为 numeric。

## 追加：4th_changerate_SAcorr（基于 ComBat-GAM 的年龄分辨 S-A 对齐）

### 目标
- 基于 ComBat-GAM 输出的 SC 数据（HCP-D/ABCD），在 1,000 个年龄点上计算 posterior derivative 与 S-A axis rank 的 Spearman rho，并输出中位数与 95% CI。
- 生成对齐曲线与 flip-age（rho≈0）分布图；同时在 SLURM log 中打印关键数值结果（flip-age 中位数/CI、rho 极值等）。

### 变更
- 新增 Rscript 入口（写入 `outputs/results/` 与 `outputs/figures/`，且在 stdout 输出 `[RESULT]` 数值行）：
...
- 新增容器提交脚本（缺失上游 derivative 文件时会自动补跑；不在会话中直接 `sbatch`）：
  - `sbatch/run_hcpd_changerate_sacorr_combatgam_CV75_container.sbatch`
  - `sbatch/run_abcd_changerate_sacorr_combatgam_CV75_container.sbatch`
- 为 ABCD 补齐生成 derivative 的上游入口（供上面 sbatch 调用）：
  - `development_script/2nd_fitdevelopmentalmodel/S1st_fitgammodels_SA_ds_sumSCinvnode_ABCD_combatgam.R`
  - `development_script/2nd_fitdevelopmentalmodel/S2nd_calculatederivative_ABCD_combatgam.R`

### 输出位置
- 结果表：`outputs/results/4th_changerate_SAcorr/{hcpd,abcd}/combat_gam/CV75/`
- 图片：`outputs/figures/4th_changerate_SAcorr/{hcpd,abcd}/combat_gam/CV75/Alignment_development/`（`.tiff + .pdf`）
- 日志：`outputs/logs/4th_changerate_SAcorr_{dataset}_CV75_<jobid>.log`
