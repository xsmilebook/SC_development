...
- `sbatch/run_abcd_pfactor_effect_continuous_container.sbatch` 增加 `/ibmgpfs` bind，并透传 `ABCD_PLOTDATASUM_RDS`（默认使用项目内 `outputs/intermediate/2nd_fitdevelopmentalmodel/abcd/combat_gam/CV75/plotdatasum.df_SA12_sumSCinvnode_siteall_CV75.rds`）。
...
## 追加：修复绘图阶段 `theme()` 报错（mvabund 覆盖 ggplot2）
### 报错
- slurm log 典型错误：`Error in theme(...): formal argument "axis.ticks" matched by multiple actual arguments`

### 根因
- `gamfunction/gamcog.R` 内部 `library(ecostats)` 会引入依赖包 `mvabund`；
- `mvabund` 提供了名为 `theme()` 的函数，可能在搜索路径上覆盖 `ggplot2::theme()`，导致 ggplot2 绘图时参数签名不匹配而崩溃。

### 修复
- 在所有会在 `source(gamcog.R)` 之后调用 `theme()`/`theme_classic()` 的 Rscript 入口中，显式固定 ggplot2 的同名函数，避免被覆盖：
  - `development_script/5th_cognition/run_abcd_cognition_comp_agecorrected_S1.R`
  - `development_script/5th_cognition/run_abcd_cognition_comp_agecorrected_S2.R`
  - `development_script/5th_cognition/run_abcd_cognition_fluid_uncorrected_S1.R`
  - `development_script/5th_cognition/run_abcd_cognition_fluid_uncorrected_S2.R`

## 追加：`nihtbx_fluidcomp_agecorrected` 新增 `meanfd_only` 版本
### 需求
- 在 ABCD fluid cognition（age-corrected）关联分析中新增 `meanfd_only` 版本（仅控制 `mean_fd`），并确保输出不覆盖默认版本。

### 实现
- `development_script/5th_cognition/run_abcd_cognition_comp_agecorrected_{S1,S2}.R` 增加 `COG_ASSOC_MODE`：
  - `original`（默认）：`s(age)+sex+mean_fd`
  - `meanfd_only`：仅 `mean_fd`（不含 `s(age)` 与 `sex`）
- 对 `meanfd_only` 若未设置 `COG_ASSOC_TAG`，输出文件名自动加 `_meanfd_only` 后缀。
- 新增一键提交脚本：`sbatch/run_abcd_cognition_comp_agecorrected_meanfd_only_container.sbatch`。

### 后续变更（2026-01-26）
- 由于 age-corrected cognition 未校正性别，默认敏感性协变量调整更新为 `sex+mean_fd`（不含 `s(age)`）：`COG_ASSOC_MODE=sex_meanfd`。
- 一键提交脚本重命名为：`sbatch/run_abcd_cognition_comp_agecorrected_sex_meanfd_container.sbatch`（仍可通过设置 `COG_ASSOC_MODE=meanfd_only` 复现旧的 `meanfd_only` 变体）。
- 若未设置 `COG_ASSOC_TAG` 且 `COG_ASSOC_MODE!=original`，输出会自动添加 `_<mode>` 后缀以避免覆盖默认版本。
