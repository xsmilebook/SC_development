...
- 在 `combat_gam/scripts/plot_abcd_variance_decomposition.R` 中增加对以下两类变体的绘制：
  ...
  - NIH Toolbox total cognition（age-corrected；`nihtbx_totalcomp_agecorrected`，baseline-only）

### 实现
- 新增输出：
  - `abcd_variance_decomp_cbcl_totprob`
  - `abcd_variance_decomp_cognition_totalcomp_agecorrected`
...
- 为区分两类 cognition 变体，`plot_abcd_variance_decomposition.R` 现在将：
  ...
  - total cognition（age-corrected）输出为 `abcd_variance_decomp_cognition_totalcomp_agecorrected`
