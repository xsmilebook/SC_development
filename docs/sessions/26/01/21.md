...
- 人口学表合并：
- 将 `demopath/nc_y_nihtb.csv` 的 `nihtbx_fluidcomp_agecorrected` 按 `src_subject_id + eventname` 左连接合并到 `demopath/DemodfScreenFinal.csv`（插入在 `nihtbx_totalcomp_uncorrected` 之后）。
...
- 现象：`Error: Missing columns: nihtbx_fluidcomp_agecorrected`（输入 RDS 缺少表型列）。
- 根因：ABCD 的 `input_rds`（`SCdata_*.merge.rds`）主要包含 SC 与基础协变量，部分表型（如 `nihtbx_fluidcomp_agecorrected`、`cbcl_scr_syn_totprob_r`）并未写入该 RDS。
- 修复：
...
- 修复：
  - `containers/scdevelopment_r41.def` 将 `ggplot2` 固定到稳定版本 `3.5.2`，并将 `patchwork` 更新到 `1.3.0`。
  - 需要在集群重新构建镜像并用 `SIF_PATH` 指向新镜像复跑。

## 追加：CBCL 关联分析切换为使用 ABCD Nonlinear-ComBat-GAM 输出
- 变更：`development_script/6th_pfactor/S2nd_cbcl_totalraw_effect_continuous_ABCD*.R` 的默认输入改为 `outputs/results/combat_gam/abcd/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.combatgam_cbcl.rds`（不新增脚本）。
- 兼容性：由于该 RDS 默认不含 `handness/race_ethnicity`，脚本会在缺列时从 `demopath/DemodfScreenFinal.csv` 按 `scanID` 自动回填（与既有 age 回填逻辑一致）。

## 追加：CBCL 全量关联 sbatch 切换为容器版并移除 smalltest
- 变更：
  - `sbatch/run_cbcl_assoc_full.sbatch` 改为使用 `outputs/containers/scdevelopment_r41.sif` 运行（可用 `SIF_PATH` 覆盖）。
  - 删除 `sbatch/run_cbcl_assoc_smalltest.sbatch`（后续如需小样本验证，建议直接在脚本内设置抽样参数或用交互式节点做少样本 run）。

## 追加：ABCD total cognition（age-corrected，baseline-only）复现入口（生成图像与 pdf）
- 输入：`outputs/results/combat_gam/abcd/SCdata_SA12_CV75_sumSCinvnode.sum.msmtcsd.combatgam_comp_agecorrected_baseline.rds`
- 复现脚本（Rscript）：
  - `development_script/5th_cognition/run_abcd_cognition_comp_agecorrected_S1.R`（关联统计 + S-A rank/矩阵图）
  - `development_script/5th_cognition/run_abcd_cognition_comp_agecorrected_S2.R`（示例散点图 + 相关分布图）
- 输出：
  - 结果：`outputs/results/5th_cognition/abcd/comp_agecorrected/`
  - 图像（tiff+pdf）：`outputs/figures/5th_cognition/abcd/comp_agecorrected/`
- 集群提交（容器版，72 核）：`sbatch sbatch/run_abcd_cognition_comp_agecorrected_container.sbatch`
- 说明：欧氏距离控制项默认读取 `wd/interdataFolder_ABCD/average_EuclideanDistance_12.csv`；也可用 `ABCD_EUCLID_CSV=/path/to/average_EuclideanDistance_12.csv` 覆盖。
- 并行说明：作业申请 72 CPU，但脚本默认最多使用 60 个 worker；若遇到进程数限制（`Cannot fork`）会按 60→50→40→30→20→… 自动降档。

## 追加：修复 cognition 统计检验在并行环境下的 `anovaPB` 崩溃（`nbinom2 not found`）
- 现象：并行运行 `gam.fit.cognition(..., stats_only=TRUE)` 时提示 `all scheduled cores encountered errors in user code`，且错误原因常为 `object 'nbinom2' not found`（来自 `ecostats::anovaPB`）。
- 根因：`anovaPB` 在部分节点/并行环境下内部并行或对象序列化不稳定，导致在 worker 中找不到依赖对象。
- 修复：`gamfunction/gamcog.R` 将 `anovaPB` 强制设置 `ncpus=1` 并对失败回退为 `NA`（后续按 `p=1` 处理），避免全边失败。
