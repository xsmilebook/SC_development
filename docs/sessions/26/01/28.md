...
## 追加：HCP-D（SA12）changerate 图风格对齐 TractSeg + control-distance 版本
- 目标：
  - 将 HCP-D（SA12）4th_changerate_SAcorr 的 S1 对齐曲线/flip-age 分布图风格与 TractSeg 版本保持一致（尺寸、配色、主题等）。
  - 额外生成 control-distance 版本：在每个年龄点先对 change rate 按边的欧氏距离做线性残差化（`derivative ~ Edistance`），再与 S-A rank 做 Spearman 相关，输出对应结果表与图像，并在日志打印 `[RESULT]` 数值。
- 修改脚本：`development_script/4th_changerate_SAcorr/S1st_SAcorr_alongAge_HCPD.R`
- 关键输出：
  - 结果：`outputs/results/4th_changerate_SAcorr/hcpd/combat_gam/CV75/alignment_*_control_distance.csv*`
  - 图像：`outputs/figures/4th_changerate_SAcorr/hcpd/combat_gam/CV75/Alignment_development/*_control_distance.{tiff,pdf}`
- 参数：
  - 欧氏距离默认读取 `wd/interdataFolder_HCPD/average_EuclideanDistance_12.csv`；可用 `--euclid_csv=/path/to.csv` 覆盖。
  - 默认不生成 svg；如确需 svg，可显式传 `--save_svg=1`（仍会输出 tiff+pdf）。

## 追加：Chinese Cohort changerate（4th_changerate_SAcorr）容器 sbatch + 数值日志输出
- 目标：为 Chinese Cohort（SA12，ComBat-GAM）提供 4th_changerate_SAcorr 的一键容器 sbatch（S1 对齐曲线 + S2 flip-age 分组 GAM），统一输出 `tiff+pdf`（不生成 svg），并在日志打印关键数值（flip-age、rho，以及分组相关 r/p）。
- 新增脚本：
  - S1：`development_script/4th_changerate_SAcorr/S1st_SAcorr_alongAge_ChineseCohort_combatgam.R`
  - S2：`development_script/4th_changerate_SAcorr/S2nd_fitgammodels_SA12sumSCinvnode_ageseperate_ChineseCohort_combatgam.R`
- 一键提交（用户在集群执行）：`sbatch sbatch/run_chinese_changerate_sacorr_combatgam_CV75_container.sbatch`

## 追加：HCP-D SA7/SA17 + SA axis multiply（SAmult）的 ComBat-GAM / devmodel / changerate 入口
- 目标：
  - 在 HCP-D 数据上新增 SA7 与 SA17 分辨率的 ComBat-GAM 输出（使用外部 merge RDS 作为输入，但输出写回本项目 `outputs/results/combat_gam/hcpd/`）。
  - 基于项目内新生成的 ComBat-GAM 输出，提供 SA7/SA17 以及 S-A axis multiply（`x*y`，记为 SAmult）三套版本的 devmodel 与 changerate 一键容器 sbatch。
  - 统一约束：不生成 svg，图像输出为 `tiff+pdf`，日志中打印关键数值（S4 的 Spearman r/p；changerate 的 flip-age 与 rho，以及分组相关 r/p）。
- 新增 ComBat-GAM sbatch（用户在集群执行）：
  - `sbatch combat_gam/sbatch/hcpd_combat_gam_sa7_sa17_CV75.sbatch`
- 新增 devmodel sbatch（容器版）：
  ...
  - SAmult：`sbatch sbatch/run_hcpd_devmodel_combatgam_CV75_SAmult_container.sbatch`
- 新增 changerate sbatch（容器版）：
  - SA7：`sbatch sbatch/run_hcpd_changerate_sacorr_combatgam_CV75_SA7_container.sbatch`
  - SA17：`sbatch sbatch/run_hcpd_changerate_sacorr_combatgam_CV75_SA17_container.sbatch`
  - SAmult：`sbatch sbatch/run_hcpd_changerate_sacorr_combatgam_CV75_SAmult_container.sbatch`
