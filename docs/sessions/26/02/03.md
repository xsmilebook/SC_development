...
- LGCM cognition/pfactor 脚本的散点图与柱状图风格对齐 LMM（点大小/线宽/主题与 stripe 低组标记）。
- LGCM cognition/pfactor 脚本新增 t 值矩阵热图与 t–S-A 相关散点图输出。
- LGCM cognition/pfactor 的 p 值改为 full vs null 的 parametric-bootstrap ANOVA，并对 edge 做 FDR 校正。
- LGCM cognition/pfactor 在 Windows 下默认启用 16 核并行（`LGCM_CORES` 可覆盖）。
- 移除 5th_cognition/6th_pfactor 脚本中的 age_to_years 逻辑（ABCD 年龄已确保为 year）。
