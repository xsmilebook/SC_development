...
- ABCD 行为变量不进入 ComBat 协变量（仅 `age/sex/mean_fd`）的敏感性分析：`sbatch sbatch/run_abcd_cognition_pfactor_no_behavior_useexisting.sbatch`。该脚本直接使用已生成的 `baseline_age_sex_meanfd` 与 `age_sex_meanfd` 输出，分别用于 cognition/pfactor 的 S1 复现（通过 `COG_ASSOC_TAG`/`PFACTOR_TAG` 避免覆盖）。
