...
5) 认知与心理病理相关分析（`development_script/5th_cognition`、`development_script/6th_pfactor`）
   ...
   - ABCD cognition（age 线性混合模型；固定效应 + 随机斜率）：
     - 模型：`SC ~ age + sex + mean_fd + (1 + age | subID)`
     - 入口脚本：`development_script/5th_cognition/run_abcd_age_lmm_random_slope_cognition_groups.R`
     - 输出：`outputs/results/5th_cognition/abcd/age_lmm/`
     - 图像：`outputs/figures/5th_cognition/abcd/age_lmm/`
