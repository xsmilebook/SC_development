...
5) 认知与心理病理相关分析（`development_script/5th_cognition`、`development_script/6th_pfactor`）
  ...
  - ABCD age_wp/age_bp LMM（cognition 版本；基线年龄分解 + 双交互 + decile 聚合 SC）：
    ...
    - 预测与图像（不输出 t 值矩阵）：
      ...
      - decile-avg-SC-first 额外统计：对 `age_wp:cov` 与 `age_bp:cov` 分别进行 LRT（`red: y ~ age_wp + cov + age_bp + sex + mean_fd + (1|subID)` vs 对应 full），并输出 FDR 校正结果到 `decile_avgSC_summary_baselineage_interaction_*.csv`；对应 FDR p 值标注在 decile 图上
